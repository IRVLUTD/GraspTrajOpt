...
class SDFCallback(Callback):
    ...
    def set_data(self, data):
        self.data = data
        self.jac_callback.data = data
        self.jac_callback.hes_callback.data = data
    ...
    def rel_pos_to_idxes(self, rel_pos):
        idx = np.floor(((rel_pos - self.origin) / self.grid_resolution)).astype(np.int32)
...
class JacFun(Callback):
    ...
    def eval(self, arg):
        ...
        out = np.stack([dx, dy, dz], axis=-1).reshape((1, 3))
        return [DM(out)]
